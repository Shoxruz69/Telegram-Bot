...
class AsyncPGWrapper:
    ...
    async def execute(self, query, params=None):
        ...
        if q.strip().upper().startswith("SELECT"):
            records = await self.conn.fetch(q, *params)
...
async def init_db():
    async with get_db() as db:
        ...
        try:
            async with db.execute("SELECT column_name FROM information_schema.columns WHERE table_name = \'users\'") as cursor:
                cols = [(0, row["column_name"]) for row in await cursor.fetchall()]
                ...
                if is_old_pk:
                    ...
                    ''')
                    await db.execute('''
                        INSERT INTO users_migrated (user_id, tenant_id, phone, latitude, longitude)
        ...
        for tbl in tables_to_check:
            try:
                async with db.execute(f"SELECT column_name FROM information_schema.columns WHERE table_name = '{tbl}'") as cursor:
        ...
        try:
            async with db.execute("SELECT column_name FROM information_schema.columns WHERE table_name = \'settings\'") as cursor:
                cols = [(0, row["column_name"]) for row in await cursor.fetchall()]
    ...
    async with get_db() as db:
        async with db.execute('SELECT phone, latitude, longitude FROM users WHERE user_id = ? AND tenant_id = ?', (user_id, tenant_id)) as cursor:
            ...
            if existing:
                ...
                await db.execute('INSERT INTO users (user_id, tenant_id, phone, latitude, longitude) ON CONFLICT (user_id, tenant_id) DO NOTHING VALUES (?, ?, ?, ?, ?)', (user_id, tenant_id, phone or '', lat or 0.0, lon or 0.0))
        await db.commit()
...
async def create_order(user_id, payment_method, receipt_image=None, tenant_id=1):
    async with get_db() as db:
        ...
        async with db.execute('''
            ...
            VALUES (?, ?, 'Kutilmoqda', ?, ?, ?) RETURNING id
        ''', (user_id, tenant_id, payment_method, receipt_image, total_amount)) as cursor:
...
async def create_custom_command(tenant_id, command, description, reply_text, reply_image=None):
    ...
    async with get_db() as db:
        async with db.execute('''
            ...
            VALUES (?, ?, ?, ?, ?) RETURNING id
        ''', (tenant_id, clean_cmd, description or '', reply_text, reply_image or '')) as cursor:
